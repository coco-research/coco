# Jason Wei — Founding Memory v2 Panel Attribution (2026-05-26 / 2026-05-27)

Jason Wei participated as a **Cell A (AI / research)** panelist in the 20-persona review of the founding Memory v1 → v2 architecture and subsequent v2 → v3 reconciliation. His role was **specialist / co-signer**, not lead-driver. He shows up in the panel artifacts on three specific stances.
...
**Source:** `the founding memory-architecture plan`, v1.6 micro-phase reasoning, and the panel synthesis section that lists Wei as co-signer.
...
**Source:** `the founding memory-architecture plan`, Section "what we kept from v2," bullet 4.
...
**Source:** `the founding memory-architecture plan`, v3.0 gate-criteria list ("SR-3 load-test report").
...
**Source:** `the founding memory-architecture plan`, v3.0 open issues, P14.
...
- "Cell A Wei + Cell B Chalef. L5 entity-extraction LLM is expensive… Gate it on cheap NER on query side." (`the founding memory-architecture plan`, v1.6)
- "Hindsight as 1-of-N behind MemoryProvider Protocol (Cell A Wei: don't single-vendor)." (`the founding memory-architecture plan`, kept-from-v2 list)
- "SR-3 load-test report with hub-and-spoke power-law-correct synthetic graph (Cell A Wei)." (`the founding memory-architecture plan`, v3.0 gate criteria)
- "CoT-retrieval interplay token budget (Cell A Wei P14)." (`the founding memory-architecture plan`, v3.0 open issues)
