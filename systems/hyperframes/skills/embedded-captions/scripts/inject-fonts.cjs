...
function loadFontLibrary() {
  ...
  try {
    ...
    // fonts.css is a generated artifact (not committed to keep base64 font
    // blobs out of the repo). Build it on demand from the committed .woff2
    // binaries in modes/standard/fonts/files/, then read it.
    try {
      require("child_process").execFileSync(
        process.execPath,
        [path.join(SKILL_ROOT, "modes", "standard", "fonts", "build-fonts-css.cjs")],
        { stdio: "ignore" },
      );
      css = fs.readFileSync(FONTS_CSS, "utf8");
    } catch {
      console.error(
        `[inject-fonts] could not build ${FONTS_CSS} — run modes/standard/fonts/build-fonts-css.cjs`,
      );
      process.exit(2);
    }
  }
