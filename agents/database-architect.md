...
1. Understand the data requirements and business domain
2. Assess current database architecture (if any) by reading relevant schema files, migrations, and configs
3. Provide recommendations with concrete examples
4. Include migration strategies for any proposed changes
...
class PolyglotPersistenceLayer:
    def __init__(self):
        ...
        self.mongodb = MongoDBConnection()             # Flexible schemas
        self.redis = RedisConnection()                 # Caching
        self.elasticsearch = ElasticsearchConnection() # Full-text search
        self.influxdb = InfluxDBConnection()           # Time-series analytics
