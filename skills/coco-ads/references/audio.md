...
Hyperframes implementation note: follow the current hyperframes audio-reactive workflow (its `references/audio-reactive.md`) to extract per-frame audio data and sample it synchronously inside the composition timeline. The extraction helper ships with the Hyperframes/GSAP skills — `/coco-ads` does not provide it, so don't hardcode a path to it.
...
skills/coco-ads/assets/music/cues/<track-stem>.music-cues.json
~/.claude/skills/coco-ads/assets/music/cues/<track-stem>.music-cues.md
~/.claude/skills/coco-ads/assets/music/cues/<track-stem>.music-cues.json
```

SFX analysis lives beside the SFX library:

```text
skills/coco-ads/assets/sfx/sfx-analysis.md
skills/coco-ads/assets/sfx/sfx-analysis.json
~/.claude/skills/coco-ads/assets/sfx/sfx-analysis.md
~/.claude/skills/coco-ads/assets/sfx/sfx-analysis.json
```

**Critical: copy audio files into the composition project before rendering.** Hyperframes validates and serves assets from the composition directory. Always copy the files you need into `coco-ads-output/composition/assets/` first:

```bash
# Create local asset dirs
mkdir -p coco-ads-output/composition/assets/sfx/interface coco-ads-output/composition/assets/sfx/impact coco-ads-output/composition/assets/sfx/casino coco-ads-output/composition/assets/sfx/ui
mkdir -p coco-ads-output/composition/assets/music
...
uv run --project skills/coco-ads/scripts \
  python skills/coco-ads/scripts/analyze_music_cues.py <track>.mp3 \
...
Wire each `<audio>` clip per the current hyperframes Data Attributes + Video/Audio contract (`data-track-index`, `data-volume`, `data-start`, `data-duration`). `/coco-ads` owns only the volume policy above and the track-allocation convention; Hyperframes owns the clip schema.
