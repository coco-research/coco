...
const FONT_FACES = (() => {
  ...
  try {
    ...
    // fonts.css is a generated artifact (not committed). Build it on demand
    // from the committed .woff2 binaries, then read it; fall back to null.
    try {
      execFileSync(process.execPath, [path.join(SKILL, "modes/standard/fonts/build-fonts-css.cjs")], { stdio: "ignore" });
      css = fs.readFileSync(path.join(SKILL, "modes/standard/fonts/fonts.css"), "utf8");
    } catch {
      return null;
    }
