...
| `--help` | Show usage |
| `--systems gsd,brain` | Install one or more system bundles. Accepted by the claude-code, grok, vscode, codex and generic adapters; the cursor adapter does not accept this flag |
...
```bash
find ~/.claude ~/.cursor ~/.grok ~/.copilot -type l -lname "*$(pwd)*" -delete
