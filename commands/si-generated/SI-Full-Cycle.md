# /SI-Full-Cycle — all-teams compound: Review → Analyse → mitigate → Decide → finalize
...
## Step 3 — Full-Cycle output
Chains the meta verbs as real invocations; each step re-orchestrates across teams; finalized action plan with per-step cross-team attribution.
...
Chain each sub-step as a real `/SI-<step>` invocation; re-orchestrate per step (teams may differ).
